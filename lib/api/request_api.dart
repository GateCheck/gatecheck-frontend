...
Future<ApiData<List<Request>>> getMultipleRequests(http.Client client,{bool messages=false, int amount=10, int index=0}){
  ...
  return client.get(url).then((res){
    return parseResponse(res, (json){
      List<Map<String, dynamic>>list=json.cast<Map<String, dynamic>>();//cast the json into a list of json objects
...
Future<ApiData<Message>> sendMessage(http.Client client, String message, Int32x4 requestId){
...
Future<ApiData<void>> deleteMessage(http.Client client, Int32x4 requestId,Int32x4 messageId){
  String url=api+"/request/${uuidToString(requestId)}/${uuidToString(messageId)}";
  return client.delete(url)
      .then(emptyResponseData);
}

Future<ApiData<void>> changeRequestStatus(http.Client client, Int32x4 requestId, RequestStatus status){
  String url=api+"/request/${uuidToString(requestId)}/status/${EnumToString.convertToString(status)}";
...
Future<ApiData<void>> updateRequest(http.Client client, Request request){
  ...
  return client.put(url,body:request.toJson())
      .then(emptyResponseData);
}